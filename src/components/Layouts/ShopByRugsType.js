...
function ShopByRugsType() {
    ...
    const type = useQuery('attr_type', async () => getAttributeByIdUrl(28), {
        onSuccess: (res) => {
                    ...
                    if (index < 12) {
                        return (
                            <li
                                ...
                                <Link href={{
                                    pathname: '/rugs',
                                    // query: { q: "type=" + item.id }
                                    query:   {typeproduct : item.id ,lable:item.label}
                                }}>
